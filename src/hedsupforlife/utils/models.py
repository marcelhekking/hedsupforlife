...
from wagtail.models import Orderable, Page, slugify
from wagtail.rich_text import expand_db_html
from wagtail.snippets.models import register_snippet
from willow.image import Image as WillowImage

from hedsupforlife.images.models import CustomImage
from hedsupforlife.utils.cache import get_default_cache_control_decorator
from hedsupforlife.utils.query import order_by_pk_position
...
# Generic social fields abstract class to add social image/text to any new content
# type easily.
...
# Generic listing fields abstract class to add listing image/text to any new content
# type easily.
class ListingFields(models.Model):
    listing_image = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        related_name="+",
        help_text="Choose the image you wish to be displayed when this page appears "
        "in listings",
    )
    ...
    listing_summary = models.CharField(
        ...
        "used as the description for search engines if the 'Meta description' "
        "field above is not defined.",
    )
...
class SystemMessagesSettings(BaseSiteSetting):
    ...
    body_404 = RichTextField(
        "Text",
        default="<p>You may be trying to find a page that doesn&rsquo;t exist "
        "or has been moved.</p>",
    )
    ...
    footer_newsletter_signup_link = models.URLField(
        blank=True,
        help_text="Link to the newsletter signup form. If left blank, the signup "
        "CTA will not be displayed.",
    )
...
class BasePage(SocialFields, ListingFields, Page):
    ...
    def plain_introduction(self):
        """
        This property returns a plain text representation of a model's 'introduction'
        field.

        If 'introduction' is a RichTextField, the method uses BeautifulSoup to filter
        and parse
        the rich text content, returning the plain text. If 'introduction' is a
        standard TextField
        or if it doesn't exist, the method returns the original text or an empty string.

        Returns:
            str: Plain text representation of the 'introduction' field or none
            if the field
