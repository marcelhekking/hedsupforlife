...
from hedsupforlife.utils.struct_values import CardStructValue, LinkStructValue
...
class AccordionBlock(blocks.StructBlock):
    heading = blocks.ListBlock(AccordionListBlock())
    list = blocks.ListBlock(AccordionListBlock())
...
class FeaturedArticleBlock(blocks.StructBlock):
    ...
    cta_text = blocks.CharBlock(
        max_length=255,
        blank=False,
        help_text="This is the cta link text",
    )
...
class BaseSectionBlock(blocks.StructBlock):
    ...
    sr_only_label = blocks.BooleanBlock(
        ...
        help_text="If checked, the heading will be hidden from view",
    )
